...
class OPHSEnv: 
    ...
    def step(self, selected) : 
        ...
        if self.current_node is not None : 
            self.last_visited_node = self.current_node.clone() + (self.hotel_number-1)
        else : 
            self.last_visited_node = torch.zeros((self.batch_size, self.pomo_size), dtype=torch.int64) + (self.hotel_number-1)
            
        self.current_node = selected.clone()
        # shape: (batch, pomo)

        self.current_node_with_hotel = self.current_node + (self.hotel_number-1)
