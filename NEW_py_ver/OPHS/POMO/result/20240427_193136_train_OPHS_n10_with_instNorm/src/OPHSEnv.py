...
class OPHSEnv: 
    ...
    def step(self, selected) : 
        ...
        if self.current_node is not None : 
            self.last_visited_node = self.current_node.clone() + self.hotel_number
        else : 
            self.last_visited_node = torch.zeros((self.batch_size, self.pomo_size), dtype=torch.int64) + self.hotel_number
            
        self.current_node = selected.clone()
        # shape: (batch, pomo)

        self.current_node_with_hotel = self.current_node + self.hotel_number
        ...
        self.first_step_len_too_large = (self.remaining_len/2 < selected_len)  
         #infeasible first step condition
    ...
    def calculate_two_distance(self) : 
        ...
        current_node_zero_indexed = self.current_node_with_hotel 
        last_visited_node_zero_indexed = self.last_visited_node 
       
        #todo 
        print(current_node_zero_indexed, last_visited_node_zero_indexed)
