...
def get_random_problems(batch_size, problem_size, hotel_size, day_number):
    ...
    remaining_len = 0.7 + (0.7 * torch.rand(batch_size, day_number))   # 0.7 to 1.4
    # remaining_len = torch.ones(size=(batch_size, day_number))
    
    node_xy = torch.rand(size=(batch_size, problem_size, 2))
    node_prize = torch.randint(1, 10, size=(batch_size, problem_size))
    
    return depot_xy, node_xy, node_prize, remaining_len
