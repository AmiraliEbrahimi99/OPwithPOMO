...
class OPHSEnv: 
    ...
    def load_problems(self, batch_size, aug_factor=1) : 
        ...
        if not self.FLAG__use_saved_problems:
            ...
            remain_len_number = 2                                                                          #define remaining length
        else:
        ...
        # shape: (batch, problem+2, 2)
        depot_prize = torch.zeros(size=(self.batch_size, 2))
        # shape: (batch, 2)
    ...
    def reset(self) : 
        ...
        # shape: (batch, pomo)
        # self.day_finished = torch.zeros(size=(self.batch_size, self.pomo_size), dtype=torch.int64)                                    #TOP
        # #shape: (batch, pomo)
    ...
    def step(self, selected) : 
        ...
        self.len_to_depot, self.len_to_depot2 = self.calculate_len_to_depot()
        # shape: (batch, pomo)

        self.first_step_len_too_large = (self.remaining_len < self.len_to_depot + self.len_to_depot2)          #infeasible first step condition
        ...
        # self.remaining_len[self.at_the_depot & (self.day_finished < 4)] = 1.5 # reset length at the depot                                         #TOP

        self.visited_ninf_flag[self.BATCH_IDX, self.POMO_IDX, selected] = float('-inf')
        # shape: (batch, pomo, problem+2)
        self.visited_ninf_flag[:, :, 1][~self.at_the_depot2] = 0  # depot 2 !!!! is considered unvisited, unless you are AT the depot
        ...
        self.len_to_depot2_expanded = self.len_to_depot2.unsqueeze(dim=1).expand(-1,self.problem_size,-1)
        # shape: (batch, pomo, problem)
        self.remaining_len_expanded = self.remaining_len.unsqueeze(dim=2).expand(-1,-1,self.problem_size)
        # shape: (batch, pomo, problem)
        
        self.possible_dists = self.len_to_depot2_expanded + self.future_len  
        self.len_too_large = self.remaining_len_expanded + round_error_epsilon < self.possible_dists
        # shape: (batch, pomo, problem)

        self.len_too_large_expanded = torch.cat((torch.zeros_like(self.len_too_large[:,:,:2], dtype=torch.bool), self.len_too_large), dim=-1)
        # shape: (batch, pomo, problem+2) 
        ...
        self.ninf_mask[:, :, 1][self.finished] = 0
    ...
    def calculate_len_to_depot(self) :
        ...
        self.depot_xy_expanded = self.depot_xy[:, 0, :].unsqueeze(1).expand(-1, self.problem_size, -1)
        #shape: (batch, problem, 2)
        self.depot2_xy_expanded = self.depot_xy[:, 1, :].unsqueeze(1).expand(-1, self.problem_size, -1)
        #shape: (batch, problem, 2)
        
        # Calculate squared differences and sum along the last dimension
        squared_diff = (self.depot_xy_expanded - self.node_xy) ** 2
        #shape: (batch, problem, 2)
        squared_diff2 = (self.depot2_xy_expanded - self.node_xy) ** 2
        #shape: (batch, problem, 2)
        ...
        #shape: (batch, problem)
        len_to_depot2 = torch.sqrt(self.distance_sums2)
        #shape: (batch, problem)

        return len_to_depot, len_to_depot2 
