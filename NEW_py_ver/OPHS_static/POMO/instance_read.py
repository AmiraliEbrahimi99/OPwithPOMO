...
def process_ophssp_file(filepath, output_dir):
    ...
    mean_rescaled = (mean_tensor - mean_min) / (mean_max - mean_min) * (0.99 - 0.02) + 0.02
    ...
    # print("Node xy rescaled tensor:", node_xy_rescaled)
    # print("Prize rescaled tensor:", prize_rescaled)
    # print(f"\n\n deviation rescaled: {deviation_rescaled}")
...
def ophssp_create_new_file(filepath, output_dir):
    ...
    deviation_generator = (torch.rand(problem_size) * (16 - 1.5) + 1.5) / 100  
    rescaled_deviation = deviation_generator * (node_prize_tensor / prize_rescaled)
    rescaled_variance = rescaled_deviation ** 2
    print(f'\n{node_prize_tensor}\n{prize_rescaled}\n{deviation_generator}\n {rescaled_deviation}\n \n')

    # Initialize the modified lines with the first line
    ...
    for i, line in enumerate(lines[(3 + hotel_size) : (3 + hotel_size + problem_size)]):
        ...
        # Format while maintaining alignment
        formatted_line = f"{x:<7} {y:<7} {z:<4d} {rescaled_variance[i].item():<7.3f}\n"
        # formatted_line = f"{float(nums[0]):<7.2f} {float(nums[1]):<7.2f} {int(nums[2]):<4d} {rescaled_variance[i].item():<7.3f}\n"
...
root_dir = "../../test_instances/OPHS_instances_February_2013/SET1 1-2/64-45-1-2.ophs"
output_dir = "../Instances"
# output_dir = "Instances/OPHSSP"
os.makedirs(output_dir, exist_ok=True)

# Iterate over all .ophs files in the directory and subdirectories
# for filepath in glob.glob(os.path.join(root_dir, "**/*.ophs"), recursive=True):
#     ophssp_create_new_file(filepath, output_dir)

ophssp_create_new_file(root_dir, output_dir)
# process_ophssp_file(root_dir2, output_dir)